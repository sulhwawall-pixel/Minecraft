...
### 🚨 리전 선택 — 인스턴스를 만들기 전에 반드시 확인

**서울(`ap-northeast-2`)** 로 맞추세요.

**AWS 콘솔 우측 상단**에 현재 리전이 표시됩니다. 여기를 클릭해서
**아시아 태평양(서울) ap-northeast-2** 를 고른 뒤에 다음 단계로 가세요.

```
        AWS 콘솔 우측 상단  ▸  [ 서울 ▾ ]  ← 여기가 "서울" 인지 확인
```

> ⚠️ **인스턴스는 나중에 다른 리전으로 옮길 수 없습니다.**
>
> 계정에서 리전을 **전환**하는 건 자유롭습니다. 하지만 **한 번 만든 인스턴스는
> 그 리전에 묶입니다.** 잘못된 리전에서 설치를 다 끝낸 뒤에 알아차리면,
> 이미지를 떠서 복사하거나(30분) 처음부터 다시 해야 합니다.
>
> 기본값이 버지니아 북부나 시드니로 되어 있는 경우가 많으니 **꼭 눈으로 확인하세요.**

리전별 한국에서의 핑:

| 리전 | 핑(대략) | |
|---|---|---|
| **아시아 태평양(서울) `ap-northeast-2`** | **~5ms** | **이걸 고르세요** |
| 아시아 태평양(도쿄) `ap-northeast-1` | ~35ms | 차선 |
| 아시아 태평양(시드니) `ap-southeast-2` | ~150ms | 플레이가 답답합니다 |
| 미국 동부(버지니아 북부) `us-east-1` | ~180ms | 안 됩니다 |

요금은 서울이 us-east-1보다 10~15% 비싸지만 이 규모에서는 월 몇 달러 차이라,
핑을 포기할 이유가 없습니다.

### 리전을 잘못 골랐다는 걸 나중에 알았다면

처음부터 다시 하지 않아도 됩니다. → [리전 옮기기](03-move-region.md)
