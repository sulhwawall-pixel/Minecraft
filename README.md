...
선릿밸리 서버 권장 RAM이 6~7GB인데, 12GB 중 **약 8GB를 힙으로** 줄 수 있습니다.
...
### 3. 홈 리전은 **도쿄**로 잡으세요 — 되돌릴 수 없습니다

가입할 때 고르는 홈 리전은 나중에 바꿀 수 없고, 무료 리소스는 홈 리전에서만
만들 수 있습니다. 그런데 **한국 리전은 둘 다 쓸 수 없습니다**:

- **서울** — 신규 무료 가입 시 홈 리전 목록에 거의 뜨지 않습니다
- **춘천** — 목록에 떠도 고르면 안 됩니다. 오라클이 무료 Ampere A1 생성 대상에서
  **공식적으로 제외한** 리전입니다

**일본 동부(도쿄, `ap-tokyo-1`)** 를 고르세요. 핑 30~40ms는 마인크래프트에서
체감되지 않습니다. 자세한 내용은 [1단계 문서](docs/01-oci-account.md#홈-리전-선택--되돌릴-수-없습니다)에 있습니다.
...
둘 다 Ubuntu라 설치 스크립트는 그대로 씁니다.

무료 플랜에서 8GB를 주는 유형은 **`m7i-flex.large` 하나뿐**이고,
AMI를 Arm으로 고르면 목록에서 사라지니 **x86 이미지**를 쓰세요.
